    ...
    if len(filter_size) == 2:
        if len(mat_size) == 3:
            ...
            for i in range(mat_size[-1]):
                pad_mat = np.pad(mat[:, :, i], ((padding[0], padding[1]), (padding[2], padding[3])),
                                 'constant')  # 默认填充0
                temp = []
...
def judgeConnect(m2, threshold):  # 连接边
    e = 0.01
    s = []
    cood = []
    for i in range(m2.shape[0]):
        ...
        for j in range(m2.shape[1]):
            ...
            if abs(m2[i, j] - 255) < e:
                s.append([i, j])  # 肯定是边的点
    cood = np.array(cood)  # [[1,1][1,2][1,3]....] [[2,1]......]

    while not len(s) == 0:
        index = s.pop()
        jud = m2[max(0, index[0] - 1):min(index[0] + 2, m2.shape[1]),
              max(0, index[1] - 1):min(index[1] + 2, m2.shape[0])]  # 边的八邻域
        ...
        jud_i = jud_i[jud]  # python数组中True和False属性，可以用一个数组来当作另一个数组的索引，可以选择去掉不要的数据，保留要的数据
...
def DecideAndConnectEdge(g_l, g_t, threshold=None):  # 非极大值抑制
    '''
...
if __name__ == '__main__':
    ...
    for i in pics:
        if i[-4:] == '.png' or i[-4:] == '.jpg' or i[-5:] == '.jpeg':
            ...
            if i[-4:] == '.png':    # png图像初始为0~1.需要和jpg、jpeg一样
                img = img * 255
